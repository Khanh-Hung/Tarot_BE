...
public class DeckController {
    ...
    public ResponseEntity<?> getAllDecks(GetDecksHandler handler) {
        return ActionResult.from(handler.handle());
    }
    ...
    public ResponseEntity<?> getCardsByDeck(
            @PathVariable DeckCode deckCode,
            GetCardsByDeckHandler handler
    ) {
        return ActionResult.from(handler.handle(deckCode));
    }
}