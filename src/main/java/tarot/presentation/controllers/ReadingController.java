...
public class ReadingController {
    ...
    public ResponseEntity<?> createReading(
            @Valid @RequestBody CreateReadingCommand command,
            CreateReadingHandler handler
    ) {
        return ActionResult.from(handler.handle(command), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<?> getReadingById(
            ...
            GetReadingDetailHandler handler
    ) {
        return ActionResult.from(handler.handle(id));
    }
    ...
    public ResponseEntity<?> getReadingHistory(
            ...
            GetReadingHistoryHandler handler
    ) {
        return ActionResult.from(handler.handle(userId, page, size));
    }
    ...
    public ResponseEntity<?> sendChatMessage(
        ...
        return ActionResult.from(handler.handle(id, command));
    }
}